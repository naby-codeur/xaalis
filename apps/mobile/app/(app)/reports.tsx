...
export default function ReportsScreen() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const response = await api.metrics.cashflow();
        if (!mounted) return;
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.card}>
        ...
        {!error && values.length === 0 ? (
          <Text style={styles.text}>Aucune metrique cashflow disponible.</Text>
        ) : null}
        {values.map((entry) => (
          <View key={entry.label} style={styles.row}>
            <Text style={styles.textStrong}>{entry.label}</Text>
            <Text style={styles.text}>{entry.value.toLocaleString("fr-FR")} XOF</Text>
          </View>
        ))}
      </View>
    </ScrollView>
